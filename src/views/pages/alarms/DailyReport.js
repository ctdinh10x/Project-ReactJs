...
const DailyReport = () => {
    ...
    const [initDayReport, setInitDayReport] = useState([])
        ...
        "data": {
            "getCoreAlarmByMonth": [
                ...
                },
                {
                    "count": "34",
                    "type": "16",
                    "time": "2023-08-12T00:00:00+00:00",
    ...
    useEffect(() => {
        let initData = initDaysAlarm()
        setInitDayReport(initData)
    }, []);
    ...
    useEffect(() => {
        ...
        let tmpDataReport = initDaysAlarm()
        console.log(tmpDataReport);
        if (tmpDataReport.length === 0) return
        let dataSearch = [];
        ...
        for (let index = 0; index < dataSearch.length; index++) {
            ...
            const date = `${day.getDate().toString().padStart(2, '0')}/${(day.getMonth() + 1).toString().padStart(2, '0')}`
            console.log(date);
            for (let index = 0; index < tmpDataReport.length; index++) {
                if (tmpDataReport[index].Time === date) {
                    tmpDataReport[index][t] = data.count;
                    tmpDataReport[index].CameraId = data.cameraId;
                    tmpDataReport[index].DeviceId = data.deviceId;
                }
            }
            // tmpDataReport[date][t] = data.count;
            // tmpDataReport[date].CameraId = data.cameraId;
            // tmpDataReport[date].DeviceId = data.deviceId;
        }
        setDayAlarm(tmpDataReport);
    }, [startDate]);
