...
const HourlyReport = () => {
    ...
    const test_number = ["16", "17", "18", "19", "15"];
    const convertedData = test_number.map(number => Alert.AlertType[number]);
    ...
    //     if (!hourly.data.getUserReportByHour) return;
    //     if (hourly.data.getUserReportByHour.length == 0) return;
    //     let tmpHourState = initHours();
    //     for (let index = 0; index < hourly?.data?.getUserReportByHour?.length; index++) {
    //         const hourData = hourly?.data?.getUserReportByHour[index];
    //         if (tmpHourState[hourData.hour]?.In !== undefined && tmpHourState[hourData.hour]?.In !== null) {
    //             tmpHourState[hourData.hour].In = hourData?.totalCheckIn ?? 0;
    //         }
    //         if (tmpHourState[hourData.hour]?.Out !== undefined && tmpHourState[hourData.hour]?.Out !== null) {
    //             tmpHourState[hourData.hour].Out = hourData?.totalCheckOut ?? 0;
    //         }
    //         if (tmpHourState[hourData.hour]?.Bothway !== undefined && tmpHourState[hourData.hour]?.Bothway !== null) {
    //             tmpHourState[hourData.hour].Bothway = hourData?.totalUndefine ?? 0;
    //         }
    //     }
    //     setHourlyRecord(tmpHourState);
    // }, [hourly]);
    useEffect(() => {
    ...
    //     {
    //         name: 'In',
    //         data: hourlyRecord.map((item) => item.In)
    //     },
    //     {
    //         name: 'Out',
    //         data: hourlyRecord.map((item) => item.Out)
    //     },
    //     {
    //         name: 'Bothway',
    //         data: hourlyRecord.map((item) => item.Bothway)
    //     }
    // ]

    const arrayOfObjects = convertedData.map(type => ({
        name: type,
